...
export interface ScalpingCompletedTrade {
  entryTime: string;
  exitTime: string;
  quantity: number;
  entryPrice: number;
...
export interface ScalpingSegmentReport {
  label: "train" | "test";
  candles: number;
  signals: number;
  ...
  totalFees: number;
  completed: ScalpingCompletedTrade[];
}

export interface ScalpingTrainTestReport {
  splitIndex: number;
  splitTime: string;
  params: ScalpingParams;
  train: ScalpingSegmentReport;
  test: ScalpingSegmentReport;
}
...
export function runScalpingTrainTest(
  candles: Candle[],
  config: Pick<
    AppConfig,
  ...
  return {
    ...
    params,
    train: runScalpingSegment("train", split.trainCandles, params, config),
    test: runScalpingSegment("test", split.testCandles, params, config)
  };
}

export function runScalpingSegment(
  label: "train" | "test",
  candles: Candle[],
  params: ScalpingParams,
  config: Pick<AppConfig, "PAPER_ACCOUNT_SIZE" | "RISK_PER_TRADE_PCT" | "PAPER_FEE_RATE" | "SLIPPAGE_PCT">
): ScalpingSegmentReport {
  const closes = candles.map((candle) => candle.close);
  const trendEma = ema(closes, params.trendEmaPeriod);
  const pullbackEma = ema(closes, params.pullbackEmaPeriod);
  const atrSeries = atr(
  ...
  const completed: ScalpingCompletedTrade[] = [];
  ...
  for (let index = 0; index < candles.length; index += 1) {
    ...
    if (pendingEntry && openPosition === null) {
      const stopDistance = pendingEntry.atrValue * params.stopAtrMultiplier;
    ...
    if (openPosition) {
      ...
      if (exitPrice !== null) {
        const trade = buildScalpingTrade(
    ...
    if (
      ...
      index < Math.max(params.trendEmaPeriod, params.pullbackEmaPeriod) ||
      atrValue === null ||
      openPosition
    ) {
      continue;
    ...
    if (isLongScalpEntry(candle, trendEma[index], pullbackEma[index])) {
      pendingEntry = { atrValue };
      signals += 1;
    }
  }
...
export function isLongScalpEntry(
  candle: Pick<Candle, "open" | "low" | "close">,
  trendEma: number | null,
  pullbackEma: number | null
) {
  return (
    trendEma !== null &&
    pullbackEma !== null &&
    candle.close > trendEma &&
    candle.low <= pullbackEma &&
    candle.close > pullbackEma &&
    candle.close > candle.open
  );
}

export function passesActiveHourFilter(
  timestamp: number,
  params: Pick<ScalpingParams, "activeHoursStartUtc" | "activeHoursEndUtc">
) {
  if (
    params.activeHoursStartUtc === undefined ||
    params.activeHoursEndUtc === undefined
  ) {
    return true;
  }
  ...
  if (params.activeHoursStartUtc <= params.activeHoursEndUtc) {
    return hour >= params.activeHoursStartUtc && hour < params.activeHoursEndUtc;
  }

  return hour >= params.activeHoursStartUtc || hour < params.activeHoursEndUtc;
}

function buildScalpingTrade(
  entryTime: string,
  entryPrice: number,
  quantity: number,
  riskAmount: number,
  ...
  } satisfies ScalpingCompletedTrade;
}
