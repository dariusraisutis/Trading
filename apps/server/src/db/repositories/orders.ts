...
export function createOrderRepository(database: DatabaseConnection) {
  const insert = database.prepare(`
    INSERT INTO orders (symbol, side, type, quantity, price, status, mode)
    VALUES (@symbol, @side, @type, @quantity, @price, @status, @mode)
  `);
  ...
  return {
    create(order: NewOrder): number {
      const result = insert.run({ ...order, price: order.price ?? null });
